...
+ (instancetype)databaseWithPath:(NSString *)aPath { return FMDBReturnAutoreleased([[self alloc] initWithPath:aPath]); }
...
- (NSString *)databasePath { return _databasePath; }
+ (NSString *)FMDBUserVersion { return @"2.5"; }
...
+ (NSString *)sqliteLibVersion { return [NSString stringWithFormat:@"%s", sqlite3_libversion()]; }
...
- (sqlite3 *)sqliteHandle { return _db; }
...
- (NSTimeInterval)maxBusyRetryTimeInterval { return _maxBusyRetryTimeInterval; }
...
- (BOOL)hasOpenResultSets { return [_openResultSets count] > 0; }
...
- (BOOL)hasDateFormatter { return _dateFormat != nil; }
...
- (NSDate *)dateFromString:(NSString *)s { return [_dateFormat dateFromString:s]; }
- (NSString *)stringFromDate:(NSDate *)date { return [_dateFormat stringFromDate:date]; }
...
- (NSString *)lastErrorMessage { return [NSString stringWithUTF8String:sqlite3_errmsg(_db)]; }
- (BOOL)hadError
{
...
- (int)lastErrorCode { return sqlite3_errcode(_db); }
...
- (NSError *)lastError { return [self errorWithMessage:[self lastErrorMessage]]; }
        ...
        if (last == '%')
        {
            switch (current)
            {
                case '@':
                    ...
                    break;
                case 'c':
                    // warning: second argument to 'va_arg' is of promotable type 'char'; this va_arg has undefined
                ...
                case 'h':
                    ...
                    {
                        //  warning: second argument to 'va_arg' is of promotable type 'short'; this va_arg has
             ...
             orVAList:(va_list)args
{
    ...
    {
        NSLog(
            @"Error: the bind count (%d) is not correct for the # of variables in the query (%d) (%@) (executeUpdate)",
            idx, queryCount, sql);
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return NO;
...
- (BOOL)shouldCacheStatements { return _shouldCacheStatements; }
