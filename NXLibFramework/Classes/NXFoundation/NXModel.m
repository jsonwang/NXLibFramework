...
- (id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    if (self)
    {
        ...
        NSDictionary *properties = [self nx_codableProperties];
        for (NSString *key in properties)
        {
            ...
            if (object)
            {
                ...
                {
                    [NSException
                         raise:SCNSObjectAutocodingException
                        format:@"Expected '%@' to be a %@, but was actually a %@", key, propertyClass, [object class]];
                }
...
{
    for (NSString *key in [self nx_codableProperties])
    {
...
- (instancetype)initWithPropertyDictionary:(NSDictionary *)dictionary
{
    ...
    {
        // Initialization code
        for (__unsafe_unretained NSString *key in [self nx_codableProperties])
        {
...
- (NSString *)JSONKeyForPropertyKey:(NSString *)propertyKey
{
    ...
    NSString *JSONKey = [JSONKeys nx_stringForKey:propertyKey];

    
    return ![NSString nx_isBlankString:JSONKey] ? propertyKey : JSONKey;
}
