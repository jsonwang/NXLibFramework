...
#import "NXAudioQueuePlayAudio.h"

static UInt32 gBufferSizeBytes = 0x10000;  // It muse be pow(2,x)
@implementation NXAudioQueuePlayAudio
...
{
    NXAudioQueuePlayAudio *player = (__bridge NXAudioQueuePlayAudio *)(inUserData);

    [(NXAudioQueuePlayAudio *)player audioQueueOutputWithQueue:inAQ queueBuffer:buffer];
}
...
- (void)audioQueueOutputWithQueue:(AudioQueueRef)audioQueue queueBuffer:(AudioQueueBufferRef)audioQueueBuffer
{
    ...
    status = AudioFileReadPackets(audioFile, NO, &numBytes, packetDescs, packetIndex, &numPackets,
...
- (id)initWithAudio:(NSString *)path
{
    ...
    {
        //错误处理
        NSLog(@"*** Error *** NXAudioQueuePlayAudio - play:Path: could not open audio file. Path given was: %@", path);
        return nil;
    }
    ...
    //创建并分配缓冲空间
    packetIndex = 0;
    for (i = 0; i < NXAudioQueuePlayAudio_NUM_BUFFERS; i++)
    {
