...
+ (NSString *)getStringFromKeychainForKey:(NSString *)keyString error:(NSError **)error
{
    ...
    NSArray *objects = [[NSArray alloc] initWithObjects:(NSString *)kSecClassGenericPassword, keyString,
                                                        [NXSystemInfo bundleID], nil];
                                    ...
                                    error:(NSError **)error
{
    ...
    if (existingString)
    {
        if (![existingString isEqualToString:savaDataString] && updateExisting)
        {
            ...
            NSArray *objects = [[NSArray alloc]
                initWithObjects:(NSString *)kSecClassGenericPassword, [NXSystemInfo bundleID],
    ...
    else
    {
        ...
        NSArray *objects = [[NSArray alloc]
            initWithObjects:(NSString *)kSecClassGenericPassword, [NXSystemInfo bundleID],
                            [NXSystemInfo bundleID], keyString,
                            [savaDataString dataUsingEncoding:NSUTF8StringEncoding], nil];
...
+ (BOOL)deleteStringFromKeychain:(NSString *)keyString error:(NSError **)error
{
    ...
    NSArray *objects = [[NSArray alloc] initWithObjects:(NSString *)kSecClassGenericPassword, keyString,
                                                        [NXSystemInfo bundleID], kCFBooleanTrue, nil];
