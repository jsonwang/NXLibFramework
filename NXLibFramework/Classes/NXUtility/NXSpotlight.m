...
+ (void)createSearchIndex:(NSArray *)dataSources
{
    ...
    [dataSources enumerateObjectsUsingBlock:^(CSSearchableItemAttributeSet *_Nonnull attributeSet, NSUInteger idx,
		...
		//这里的UniqueIdentifier的值 在点击后通过kCSSearchableItemActivityIdentifier KEY 可以取成,如设置vid=5200&uid=1000 为跳转界面使用
    ...
    // XXXX:索引的创建及删除都是在一个子线程里 在子线程里也就说明我们无法控制其执行的先后顺序
...
[[CSSearchableIndex defaultSearchableIndex] deleteSearchableItemsWithIdentifiers:[NSArray
arrayWithObjects:@"apple-001", @"apple-002", nil nil] completionHandler:^(NSError * _Nullable error) {
	if (!error) {
		NSLog(@"Items deleted");
	}
}];

[[CSSearchableIndex defaultSearchableIndex] deleteSearchableItemsWithDomainIdentifiers:[NSArray
arrayWithObjects:@"xxx.fruit.com", nil nil] completionHandler:^(NSError * _Nullable error) {
	if (!error) {
		NSLog(@"Items deleted");
	}
}];
