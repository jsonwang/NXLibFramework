...
// xxxx add by ak 这个类不能实现 NSKeyedUnarchiverDelegate NSKeyedArchiverDelegate 会查找属性列表 找到OBJECT 协议的属性
// 导致CRASH
...
- (id)initWithCoder:(NSCoder *)coder
{
    ...
    while (cls != [NSObject class])
    {
        ...
        Ivar *ivarList = bIsSelfClass ? class_copyIvarList([cls class], &iVarCount)
        : NULL;  //*变量列表，含属性以及私有变量 (包括在@interface大括号中声明的变量)
...
{
    return
    [[NXFileManager getDocumentDir] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.zip", key]];
}
