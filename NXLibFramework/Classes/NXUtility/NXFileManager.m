...
+ (NSMutableArray *)absoluteDirectories
{
    ...
    dispatch_once(&token, ^{
        ...
        directories = [NSMutableArray
            arrayWithObjects:[self pathForApplicationSupportDirectory], [self pathForCachesDirectory],
...
+ (BOOL)createDirectoriesForFileAtPath:(NSString *)path error:(NSError **)error
{
    ...
    return
        [self createDirectoriesForPath:[[self absolutePath:path] stringByDeletingLastPathComponent] error:error];
}
...
{
    return ([self isFileItemAtPath:path error:error] &&
