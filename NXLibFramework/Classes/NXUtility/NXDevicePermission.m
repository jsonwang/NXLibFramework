...
+ (void)requestSocialAccountAuthor:(NSString *)accountTypeIndentifier
{
    ACAccountStore *accountStore = [[ACAccountStore alloc] init];
                                           ...
                                           dispatch_async(dispatch_get_main_queue(), ^{
...

                                               {
                                                   NSArray *arrayOfAccounts = [accountStore
                                                       ...
                                                       accountsWithAccountType:sinaWeiboAccount];

                                                   if ([arrayOfAccounts count] > 0)

                                                   {
                                                       ACAccount *sinaWeiboAccount = [arrayOfAccounts objectAtIndex:0];
                                                       NSLog(@"第一个账号是%@", sinaWeiboAccount.identifier);
                                                       // more 发送一个微博
                                                   }
                                               };
