    ...
    @synchronized(appRegisteredCustomFonts)
    {
        ...
        if (fontPSNames == nil)
        {
                ...
                if (fontDescriptors)
                {
                    ...
                    for (NSDictionary *fontDescriptor in fontDescriptors)
                    {
                        ...
                        if (fontPSName)
                        {
                            ...
                            {
                                NSLog(
                                    @"Warning with font registration: Font '%@' already registered", fontPSName);
                            }
                    ...
                    {  // [fontPSNames count] <= 0
                        NSLog(
                            @"Warning with font registration: All fonts in '%@' are already registered", fontURL);
                    }
                }
                else
            ...
            {  // [UIFont deviceHasFullSupportForFontCollections] fail
                ...
                if (fontData)
                {
                    ...
                    if (loadedFont != NULL)
                    {
                        ...
                        if ([singleFontValidExtensions containsObject:[fontURL pathExtension]])
                        {
                            ...
                            {
                                NSLog(
                                    @"Warning with font registration: All fonts in '%@' are already registered",
                                    fontURL);
                            }
                        }
                        else
                ...
                {
                    NSLog(@"Error with font registration: URL '%@' cannot be read with error: %@",
                                             fontURL, error);
                    fontPSNames = nil;
                }
            }
